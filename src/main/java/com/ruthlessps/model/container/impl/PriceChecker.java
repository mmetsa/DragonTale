...
public class PriceChecker extends ItemContainer {
	...
	public String calculateTotalWealth() {
		int k = 0;
		...
		if(k <= 1000) {
			...
			if(Integer.toString(k).substring(1) == "000") {
				return k/1000+" @cya@Bil@whi@   "+Integer.toString(k).substring(1)+" @gre@Mil";
			} else {
				return k/1000+" @cya@Bil@whi@";
			}
		} else if(k <= 100000) {
			if(Integer.toString(k).substring(2) == "000") {
				return k/1000+" @cya@Bil@whi@   "+Integer.toString(k).substring(2)+" @gre@Mil";
			} else {
				return k/1000+" @cya@Bil@whi@";
			}
		} else if(k <= 1000000) {
			if(Integer.toString(k).substring(3) == "000") {
				return k/1000+" @cya@Bil@whi@   "+Integer.toString(k).substring(3)+" @gre@Mil";
			} else {
				return k/1000+" @cya@Bil";
			}
		} else if(k <= 10000000) {
			return k/1000000+" @blu@Tril@whi@   "+Integer.toString(k).substring(1, 4)+" @cya@Bil";
		} else if(k <= 100000000) {
			return k/1000000+" @blu@Tril@whi@   "+Integer.toString(k).substring(2, 5)+" @cya@Bil";
		} else if(k <= 1000000000) {
			return k/1000000+" @blu@Tril@whi@   "+Integer.toString(k).substring(3, 6)+" @cya@Bil";
		} else if(k > 1000000000 && k < 2147000000) {
			return k/1000000000+" @bla@Q@whi@   "+Integer.toString(k).substring(1, 4)+" @blu@Tril @whi@  "+Integer.toString(k).substring(4, 7)+" @cya@Bil";
		} else {
			return "Too High!";
		}
	}
	...
	public PriceChecker refreshItems() {
		...
		for (int i = 0; i < frames.length; i++) {
			...
			getPlayer().getPacketSender().sendString(18351, "" + calculateTotalWealth());
		}
		if (itemFound) {
