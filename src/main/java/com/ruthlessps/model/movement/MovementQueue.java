package com.ruthlessps.model.movement;
...
import com.ruthlessps.model.movement.path.PathGenerator;
	...
	public void sequence() {
		...
		if (character.isPlayer()) {
			...
			if (movementOccured) {
				...
				if(p.getLocation() == Location.BUNNYHOP) {
					if(p.getBunnyhop().checkBad()) {
						p.getMovementQueue().setLockMovement(true);
						p.performAnimation(new Animation(7253));
						p.performGraphic(new Graphic(77));
						p.delayedMoveTo(new Position(2509, 4759, 0), 2);
