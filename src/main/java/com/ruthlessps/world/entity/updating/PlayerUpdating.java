package com.ruthlessps.world.entity.updating;

import java.util.Iterator;

import com.ruthlessps.model.Appearance;
...
import com.ruthlessps.model.definitions.ItemDefinition.EquipmentType;
...
import com.ruthlessps.net.packet.PacketBuilder.AccessType;
...
public class PlayerUpdating {
	...
	private static void updateAppearance(Player player, PacketBuilder out, Player target) {
		...
		if (player.getNpcTransformationId() <= 0) {
			...
			if (ItemDefinition.forId(equip[Equipment.HEAD_SLOT]).getEquipmentType().equals(EquipmentType.FULL_HELMET)
					|| ItemDefinition.forId(equip[Equipment.HEAD_SLOT]).getEquipmentType()
							.equals(EquipmentType.FULL_MASK)
					|| ItemDefinition.forId(equip[Equipment.HEAD_SLOT]).getEquipmentType()
							.equals(EquipmentType.HALF_MASK)) {
				properties.put(0);
			} else {
			...
			if (appearance.getLook()[Appearance.BEARD] <= 0 || appearance.getGender().equals(Gender.FEMALE)) {
				...
				if (ItemDefinition.forId(equip[Equipment.HEAD_SLOT]).getEquipmentType()
						.equals(EquipmentType.FULL_HELMET)
						|| ItemDefinition.forId(equip[Equipment.HEAD_SLOT]).getEquipmentType()
								.equals(EquipmentType.FULL_MASK)
						|| ItemDefinition.forId(equip[Equipment.HEAD_SLOT]).getEquipmentType()
								.equals(EquipmentType.BEARD_MASK)) {
					properties.put(0);
				} else {
		...
		properties.putShort(target.getDonor().ordinal());
	...
	private static void updateChat(PacketBuilder builder, Player target) {
		...
		builder.put(target.getDonor().ordinal());
		builder.put(target.isModeler() ? 1 : 0);
		builder.put(target.isGambler() ? 1 : 0);
		builder.put(target.isGfxDesigner() ? 1 : 0);
		builder.put(target.isYoutuber() ? 1 : 0);
	...
	private static void updateForcedMovement(Player player, PacketBuilder builder, Player target) {
		...
		builder.putShort(target.getForceMovement().getReverseSpeed(), ValueType.A);
		// builder.putShort(target.getForceMovement().getAnimation(), ValueType.A,
		// ByteOrder.LITTLE);
