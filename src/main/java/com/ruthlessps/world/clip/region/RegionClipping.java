package com.ruthlessps.world.clip.region;
...
import com.ruthlessps.world.clip.stream.ByteStream;
import com.ruthlessps.world.entity.impl.Character;

/**
 * A highly modified version of the released clipping.
 * 
 * @author Relex lawl and Palidino: Gave me (ruthless) the base.
 * @editor ruthless: Rewrote the system, now loads regions when they're actually
 *         needed etc.
 */
public final class RegionClipping {
	...
	public static int getClipping(int x, int y, int height) {
		...
		if (height >= 4)
			height = 0;
				...
				|| Location.inLocation(x, y, Location.GAMBLING_ZONE))
			return 0;
		RegionClipping r = get(regionId);
		if (r != null) {
	...
	public static boolean objectExists(GameObject object) {
		...
		boolean crystalChest = id == 172 && pos.getX() == 2909 && pos.getY() == 2712;
		if (id == 23271) {
			return true;
		}
		if (well || mageBankLever || lawAltar || trees || chaosTunnels || lunar || rfd || lumbridgeCastle
				|| barbCourseRopeswing || catherbyAquariums || freeForAllPortal || godwars || waterRcAltar
				|| crystalChest)
			return true;
