...
public enum CombatSpecial {
	...
	ABYSSAL_WHIP(new int[] { 4151, 19810, 11529, 11526, 11527, 11528, 9707 }, 50, 1, 1, CombatType.MELEE,
	...
	MAGIC_SHORTBOW(new int[] { 861, 3083 }, 55, 1, 1.2, CombatType.RANGED, WeaponInterface.SHORTBOW) {
		@Override
		public CombatContainer container(Player player, Character target) {
	...
	DARK_BOW(new int[] { 11235, 11288, 11289, 11290, 11291, 3078 }, 55, 1.45, 1.22, CombatType.RANGED,
		...
		public CombatContainer container(Player player, Character target) {
			...
			return new CombatContainer(player, target, 2, CombatType.RANGED, true);
		}// this is what calls it tht right testing it to make sure
	},
		...
		public CombatContainer container(Player player, Character target) {
			...
			TaskManager.submit(new Task(1, player, false) {
				...
				public void execute() {
					...
					new CombatHitTask(player.getCombatBuilder(),
	 ...
	 *            <code>null</code> if the player is not in combat while activating
	 *            the special bar.
	 */
	public void onActivation(Player player, Character target) {
