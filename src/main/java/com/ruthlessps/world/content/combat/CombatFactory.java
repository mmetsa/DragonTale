...
import com.ruthlessps.model.movement.PathFinder;
import com.ruthlessps.util.Misc;
import com.ruthlessps.world.clip.region.RegionClipping;
import com.ruthlessps.world.content.BonusManager;
import com.ruthlessps.world.content.ItemDegrading;
import com.ruthlessps.world.content.ItemDegrading.DegradingItem;
...
import com.ruthlessps.world.content.combat.magic.CombatAncientSpell;
...
public final class CombatFactory {
	...
	public static final long DAMAGE_CACHE_TIMEOUT = 600000;
	...
	protected static void applyPrayerProtection(CombatContainer container, CombatBuilder builder) {
		...
		if (builder.getVictim().isPlayer()) {
			...
			if (victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 13738
					|| victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 13740
					|| victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 13742
					|| victim.getEquipment().getItems()[Equipment.SHIELD_SLOT].getId() == 13744) {
	...
	public static int calculateMaxRangedHit(Character entity, Character victim) {
		...
		double rangedBoost = 1;

		int rangedStrength = ((int) player.getBonusManager().getAttackBonus()[4] / 5);
		...
		if (victim != null && victim.isNpc()) {
			...
			if (npc.getId() == player.getSlayer().getSlayerTask().getNpcId()) {
				rangedBoost = DesolaceFormulas.getBoost(player, rangedBoost);
			}
		}
		rangedBoost *= player.getAttributes().getRangedMultiplier() - 1;
		maxHit *= rangedBoost;
		maxHit *= 10;
		if(player.getAttributes().getPet().getId() == 4201) {
			maxHit = (int) (maxHit * 1.35);
		}else if(player.getAttributes().getPet().getId() == 13453 || player.getAttributes().getPet().getId() == 5268) {
	...
	public static boolean checkAttackDistance(Character a, Character b) {
		...
		/*
		 * Clipping checks and diagonal blocking by ruthless
		 */

		boolean sameSpot = attacker.equals(victim) && !a.getMovementQueue().isMoving()
				&& !b.getMovementQueue().isMoving();
		//!sameSpot &&
		boolean goodDistance = Locations.goodDistance(attacker.getX(), attacker.getY(), victim.getX(), victim.getY(), distance);
	...
	public static boolean checkHook(Character entity, Character victim) {
		...
		if(entity.isPlayer()) {
			...
			if(player.getLocation() == Location.WILDERNESS) {
				if(player.getEquipment().containsAny(GameSettings.BANNEDWILD_ITEMS)) {
					player.moveTo(new Position(3041, 2855, 0));
					player.sendMessage("@red@Do not try to use banned wild items in wildy!");
				}
			}
		}
		if (victim.isNpc() && entity.isPlayer()) {
			...
			if (npc.isSummoningNpc() && npc.getSpawnedFor().getAttributes().getPet() != null) {
				Player player = ((Player) entity);
				if (player.getLocation() != Location.WILDERNESS) {
			...
			} else if (npc.getId() == 8549) { // Phoenix
				if (entity.getCombatBuilder().getStrategy().getCombatType() == CombatType.MELEE) {
					entity.getCombatBuilder().reset(true);
					return false;
				}
			} else if (npc.getId() == 7235) { // Phoenix
				if (entity.getCombatBuilder().getStrategy().getCombatType() == CombatType.MELEE) {
					((Player) entity).getPacketSender().sendMessage("This monster is resistant to melee attacks.");
					entity.getCombatBuilder().reset(true);
					return false;
				}
			} else if (npc.getId() == 1860) { // Phoenix
				if (entity.getCombatBuilder().getStrategy().getCombatType() == CombatType.MELEE) {
					((Player) entity).getPacketSender().sendMessage("This monster is resistant to melee attacks.");
					entity.getCombatBuilder().reset(true);
					return false;
				}
			} else if (npc.getId() == 3712 && !((Player) entity).getEquipment().containsAny(3637, 3638, 3640)) { // Phoenix
					((Player) entity).getPacketSender().sendMessage("You need Happy weaponry to kill these NPC's.");
					entity.getCombatBuilder().reset(true);
					return false;
			} else if (npc.getId() == 4500 && !((Player) entity).getEquipment().containsAny(3637, 3638, 3640)) { // Phoenix
					((Player) entity).getPacketSender().sendMessage("You need Happy weaponry to kill these NPC's.");
					entity.getCombatBuilder().reset(true);
					return false;
			} else if (npc.getId() == 1871 && !((Player) entity).getEquipment().containsAny(82, 894, 895)) {
					((Player) entity).getPacketSender().sendMessage("You need elemental whips to kill these NPC's.");
					entity.getCombatBuilder().reset(true);
					return false;
			} else if (npc.getId() == 1388 && !((Player) entity).getEquipment().containsAny(82, 894, 895)) { // Phoenix
					((Player) entity).getPacketSender().sendMessage("You need elemental whips to kill these NPC's.");
					entity.getCombatBuilder().reset(true);
					return false;
			}
		...
		//entity.getCombatBuilder().getLastAttacker() != null
		...
		if (!(entity.isNpc() && ((NPC) entity).isSummoningNpc())) {
			...
			if (victim.getCombatBuilder().getLastAttacker() != null && !Location.inMulti(entity)
				...
				if (!allowAttack) {
					...
					entity.getCombatBuilder().cooldown(true);
					return false;
				}
			}
		}
		...
		if (entity.isNpc()) {
			NPC n = (NPC) entity;
			if (!Location.ignoreFollowDistance(n) && !n.isSummoningNpc()) { // Stops
																			// combat
																			// for
																			// npcs
																			// if
				...
				if (!n.getPosition().isWithinDistance(n.getDefaultPosition(),
					...
					n.moveTo(new Position(n.getDefaultPosition().getX(), n.getDefaultPosition().getY()));
					entity.getCombatBuilder().cooldown = 10;
					entity.getMovementQueue().setFollowCharacter(null);
					return false;
				}
	...
	public static Hit getHit(Character entity, Character victim, CombatType type) {
		...
		case MELEE:
			return new Hit(
					Misc.inclusiveRandom(1, /* CombatFactory.calculateMaxMeleeHit(entity, victim) */ DesolaceFormulas
		...
		case RANGED:
			return new Hit(Misc.inclusiveRandom(1, CombatFactory.calculateMaxRangedHit(entity, victim)), Hitmask.RED,
					CombatIcon.RANGED);
		case MAGIC:
			return new Hit(Misc.inclusiveRandom(1, DesolaceFormulas.getMagicMaxhit(entity, victim)), Hitmask.RED,
					CombatIcon.MAGIC);
		case DRAGON_FIRE:
	...
	public static int getLevelDifference(Player player, boolean up) {
		...
		int wildLevel = player.getWildernessLevel() + 5; // + 5 to make wild
															// more active
	...
	protected static void handleArmorEffects(Character attacker, Character target, int damage, CombatType combatType) {
		...
		if (Misc.exclusiveRandom(4) == 0) {
			 ...
			 * 1, 10)); Skills.refresh(victim, Skills.STRENGTH); victim.performGraphic(new
			 * Graphic(400)); return; }
			 * 
	...
	protected static void handlePrayerEffects(Character attacker, Character target, int damage, CombatType combatType) {
		...
		if (attacker.isPlayer()) {
			...
			if ((CurseHandler.isActivated(p, CurseHandler.SOUL_SPLIT) && damage > 0) || p.getAttributes().getPet().getId() == 539) {
	...
	public static boolean fullKM(Character entity) {
		return ((Player) entity).getEquipment().containsAll(16639, 16640, 16641, 16644);
	}
}
