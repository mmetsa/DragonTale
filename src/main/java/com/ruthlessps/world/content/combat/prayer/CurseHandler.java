package com.ruthlessps.world.content.combat.prayer;
...
public class CurseHandler {
	...
	private enum CurseData {
		PROTECT_ITEM(50, 4, 32503, 610, new PrayerAnimation(new Animation(12567), new Graphic(2213))),
		SAP_WARRIOR(50, 4, 32505, 611), 
		SAP_RANGER(52, 4, 32507, 612), 
		SAP_MAGE(54, 4, 32509, 613), 
		SAP_SPIRIT(56, 4, 32511, 614), 
		BERSERKER(59, 6, 32513, 615, new PrayerAnimation(new Animation(12589),new Graphic(2266))),
		DEFLECT_SUMMONING(62, 6, 32515, 616), 
		DEFLECT_MAGIC(65, 6, 32517, 617), 
		DEFLECT_MISSILES(68, 6, 32519, 618), 
		DEFLECT_MELEE(71, 6, 32521, 619), 
		LEECH_ATTACK(74, 8, 32523, 620), 
		LEECH_RANGED(76, 8, 32525, 621), 
		LEECH_MAGIC(78, 8, 32527, 622), 
		LEECH_DEFENCE(80, 8, 32529, 623), 
		LEECH_STRENGTH(82, 8, 32531, 624), 
		LEECH_ENERGY(84, 10, 32533, 625), 
		LEECH_SPECIAL_ATTACK(86, 10,32535, 626), 
		WRATH(89, 10, 32537,627), 
		SOUL_SPLIT(92, 10, 32539,628), 
		TURMOIL(95, 10,32541, 629,new PrayerAnimation(new Animation(12565),new Graphic(2226)));
	...
	public static void activateCurse(Player player, CurseData curse) {
		...
		if (player.getSkillManager().getMaxLevel(Skill.PRAYER) < (curse.requirement * 10)) {
			...
			// @blu@Prayer <col=0>level of " + pd.levelRequirement +" to use
			// @blu@" + pd.getPrayerName());
	...
	private static final double getDrain(Player player) {
		...
		if (toRemove > 0) {
			//toRemove /= (1 +player.getBonusManager().getOtherBonus()[2])*90;
			//1-20
			for(int i = 0; i < player.getBonusManager().getOtherBonus()[2]+player.getAttributes().getPrayerBonus(); i+=100) {
				toRemove *= 0.9;
			}
		}
		return toRemove;
		...
		TaskManager.submit(new Task(1, player, true) {
			...
			public void execute() {
				if ((player.getDonor() == PlayerRanks.DonorRights.SPONSOR
						...
						|| player.getDonor() == PlayerRanks.DonorRights.VETERAN
						|| player.getDonor() == PlayerRanks.DonorRights.VIP
						|| player.getDonor() == PlayerRanks.DonorRights.KING
						|| player.getDonor() == PlayerRanks.DonorRights.SUPER_SPONSOR
								|| player.getDonor() == PlayerRanks.DonorRights.HEARTH
										|| player.getDonor() == PlayerRanks.DonorRights.GOLDBAG
												|| player.getDonor() == PlayerRanks.DonorRights.GEM
						|| player.getDonor() == PlayerRanks.DonorRights.DRAGON|| player.getInfinitePrayer()) && player.getLocation() != Location.WILDERNESS)
					return;
				if (player.getSkillManager().getCurrentLevel(Skill.PRAYER) <= 0) {
					for (CurseData curse : CurseData.values()) {
						if (player.getCurseActive()[curse.ordinal()]) {
				...
				if (player.getLocation() != Location.WILDERNESS) {
					if (player.getDonor() == PlayerRanks.DonorRights.SPONSOR
							...
							|| player.getDonor() == PlayerRanks.DonorRights.VIP
							|| player.getDonor() == PlayerRanks.DonorRights.KING
							|| player.getDonor() == PlayerRanks.DonorRights.DRAGON
							|| player.getDonor() == PlayerRanks.DonorRights.HEARTH
							|| player.getDonor() == PlayerRanks.DonorRights.GOLDBAG
							|| player.getDonor() == PlayerRanks.DonorRights.GEM) {
						drain = 0;
