	...
	public static void bindItem(Player p, int item) {
		if (Dungeoneering.doingDungeoneering(p)) {
			...
			if (amountBound >= 4) {
				...
				return;
			} else if (amountBound == 3 && p.getSkillManager().getCurrentLevel(Skill.ATTACK) < 120) {
				p.getPacketSender()
						.sendMessage("You need a Dungeoneering level of at least 120 to have 4 bound items.");
				return;
			} else if (amountBound == 2 && p.getSkillManager().getCurrentLevel(Skill.ATTACK) < 85) {
				p.getPacketSender().sendMessage("You need a Dungeoneering level of at least 85 to have 3 bound items.");
				return;
			} else if (amountBound == 1 && p.getSkillManager().getCurrentLevel(Skill.ATTACK) < 50) {
