...
public class Summoning {
	...
	public void processFamiliar() {
		...
		TaskManager.submit(new Task(1, n, true) {
			...
			protected void execute() {
				...
				n.getDefinition().setAttackable(true);
				if (n.getDefinition().isAttackable() && (underAttack || attacking)) {
	...
	public void summon(final FamiliarData familiar, boolean renew, boolean login) {
		...
		if (login || player.getSkillManager().getMaxLevel(Skill.SUMMONING) >= familiar.levelRequired) {
			...
			World.register(foll);
			Achievements.doProgress(player, AchievementData.SUMMON_FAMILIAR);
			Achievements.doProgress(player, AchievementData.SUMMON_25_FAMILIARS);
			Achievements.doProgress(player, AchievementData.SUMMON_100_FAMILIARS);
	...
	public void summonPet(Pet pet, boolean login) {
		if(player.getAttributes().getPet().getId() != 0) {
			player.getPA().sendMessage("Please dismiss your familiar first.");
			return;
		}
		if (!player.getLocation().isSummoningAllowed()) {
			player.getPacketSender().sendMessage("You cannot summon familiars here.");
		...
		NPC foll = new NPC(pet.getId(), spawn);
		player.getAttributes().setPet(pet);
		initFollower(foll, foll.getDefinition().getName());
	}

	private void initFollower(NPC foll, String name) {
		foll.performGraphic(new Graphic(1315));
		foll.setPositionToFace(player.getPosition());
		foll.setSummoningNpc(true);
		foll.setEntityInteraction(player);
		...
		processFamiliar();
		if(player.getAttributes().getPet() != null && player.getAttributes().getPet().getId() == 4972) {
			WeaponAnimations.assign(player, new Item(12508));
			player.getUpdateFlag().flag(Flag.APPEARANCE);
		}
		player.getPacketSender().sendString(54028, "" + name.replaceAll("_", " "));
		player.getPacketSender().sendString(54045, " " + player.getSkillManager().getCurrentLevel(Skill.SUMMONING) + "/"
				+ player.getSkillManager().getMaxLevel(Skill.SUMMONING));
		player.getPacketSender().sendString(0, "[SUMMOtrue");
		player.getPacketSender().sendString(54043, "");
	...
	private Position getPosition() {
		Position spawn = new Position(player.getPosition().getX(), player.getPosition().getY() + 1,
				player.getPosition().getZ());
		if (player.getMovementQueue().canWalk(-1, 0)) {
			spawn = new Position(player.getPosition().getX() - 1, player.getPosition().getY(),
					player.getPosition().getZ());
		} else if (player.getMovementQueue().canWalk(1, 0)) {
			spawn = new Position(player.getPosition().getX() + 1, player.getPosition().getY(),
					player.getPosition().getZ());
		} else if (player.getMovementQueue().canWalk(0, -1)) {
			spawn = new Position(player.getPosition().getX(), player.getPosition().getY() - 1,
					player.getPosition().getZ());
		} else if (player.getMovementQueue().canWalk(0, 1)) {
			spawn = new Position(player.getPosition().getX(), player.getPosition().getY() + 1,
					player.getPosition().getZ());
	...
	public void summonPet(BossPet bossPet, boolean login) {
		...
		NPC foll = new NPC(bossPet.spawnNpcId, spawn);
		initFollower(foll, bossPet.name());
	}
