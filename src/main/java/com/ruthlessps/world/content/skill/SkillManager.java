package com.ruthlessps.world.content.skill;
...
import com.ruthlessps.world.content.BrawlingGloves;
...
public class SkillManager {
	 ...
	 */
	public static int getMaxAchievingLevel(Player player,Skill skill) {
		int level = 99;
		if (isNewSkill(skill)) {
			level = 990;
		}
	...
	public SkillManager addExperience(Skill skill, int experience) {
		...
		if(player.getEquipment().contains(18405)) {
			experience *= 2.0;
		}
		if(player.getEquipment().contains(18406)) {
			experience *= 3.0;
		}
		if(player.getEquipment().contains(18407)) {
			experience *= 4.0;
		}
		...
		experience *= player.getMultiplier();
		...
		if (newLevel > startingLevel) {
			...
			if(skill != Skill.ATTACK && skill != Skill.DEFENCE && skill != Skill.STRENGTH && skill != Skill.MAGIC && skill != Skill.RANGED && skill != Skill.CONSTITUTION&& skill != Skill.PRAYER) {
				int amt = 0;
				if(newLevel-startingLevel > 1) {
					amt = 10*(newLevel-startingLevel);
					if(startingLevel < 30 && newLevel > 30)  {
						amt = 300;
					} else if(startingLevel < 20 && newLevel > 20)  {
						amt = 200;
					} else if(startingLevel < 10 && newLevel > 10)  {
						amt = 100;
					}
				} else if(newLevel == 10) {
					amt = 100;
				...
				if(player.getAttributes().getPet().getId() == 3249) {
					player.getPointsManager().setWithIncrease("skill", amt);
				}
			}
			PlayerPanel.refreshPanel(player);
			Sounds.sendSound(player, Sound.LEVELUP);
			if (skills.maxLevel[skill.ordinal()] == getMaxAchievingLevel(player, skill)) {
				player.getPacketSender()
						.sendMessage("Well done! You've achieved the highest possible level in this skill!");
				if(player.getGameMode() == GameMode.HARDCORE && skill != Skill.ATTACK && skill != Skill.DEFENCE && skill != Skill.STRENGTH && skill != Skill.MAGIC && skill != Skill.RANGED && skill != Skill.CONSTITUTION&& skill != Skill.PRAYER) {
					player.getInventory().add(15501, 2);
				}
				if(skill == Skill.PRAYER) {
					Achievements.doProgress(player, AchievementData.GET_99_PRAYER);
				}
			...
			if (getCombatLevel() > 50) {
				Achievements.doProgress(player, AchievementData.REACH_COMBAT_LVL_50);
			}
			if (getCombatLevel() > 75) {
				Achievements.doProgress(player, AchievementData.COMBAT_LVL_75);
			}
			if (getCombatLevel() == 138) {
				Achievements.doProgress(player, AchievementData.MAX_COMBAT);
			}

			if (getTotalSkillsMastered() == MAX_SKILLS) {
				Achievements.doProgress(player, AchievementData.MAX_SKILL);
	...
	public SkillManager resetSkill(Skill skill, boolean prestige) {
		...
		if (prestige && player.getSkillManager().getMaxLevel(skill) < getMaxAchievingLevel(player, skill)) {
			player.getPacketSender()
		...
		if (prestige) {
			...
			if(player.getPrestigeCount() == 5) {
				player.setPrestigeCount(0);
				player.setMultiplier(player.getMultiplier()*0.99);
				player.getPA().sendMessage("<img=10><col=FF7400><shad=0>Your XP multiplier is now "+(double) Math.round(player.getMultiplier() * 100) / 100);
			}
			int pts = getPrestigePoints(player, skill);
			if(skill == Skill.SLAYER) {
				player.setSlayerPrestige(player.getSlayerPrestige()+1);
			}
			player.getPointsManager().setWithIncrease("prestige", pts);
			player.getPacketSender().sendMessage("<img=10><col=FF7400>You've received some prestige points!");
			player.getPointsManager().refreshPanel();
			if(player.getGameMode() == GameMode.HARDCORE) {
				if(player.getPrestigeAmount()%50 == 0) {
					String playername = player.getUsername().substring(0, 1).toUpperCase()+player.getUsername().substring(1, player.getUsername().length());
					World.sendMessage("<img=10><col=FF7400> @red@"+playername+"<col=FF7400> has just prestiged @red@"+skill.getName()+"<col=FF7400> and has prestiged @red@"+player.getPrestigeAmount()+"<col=FF7400> times in total.");
				}
				}
		} else {
