...
import com.ruthlessps.engine.task.impl.PoisonImmunityTask;
...
public class Consumables {
	...
	private enum FoodType {
		...
		KEBAB(new Item(1971), 40), CHEESE(new Item(1985), 45), CAKE(new Item(1891), 50), SECOND_CAKE_SLICE(
				new Item(1893),
				50), THIRD_CAKE_SLICE(new Item(1895), 50),STARTER_BANDAGES(new Item(4049), 120), JANGERBERRIES(new Item(247),
	...
	public static void drinkLifestealPotion(final Player player, int potion, int replacePotion, int slot, int skill,
			boolean super_pot) {
		if (slot >= 0) {
			player.performAnimation(new Animation(829));
			player.getInventory().getItems()[slot] = new Item(replacePotion, 1);
		...
		player.getInventory().refreshItems();
		player.getAttributes().setLifestealTimer(3000);
		player.getPacketSender().sendMessage("<img=10><col=FF7400><shad=0>You have been granted lifesteal boost for 30 minutes.");
	}

	public static void drinkDrPotion(final Player player, int potion, int replacePotion, int slot, int skill,
			boolean super_pot) {
		if (slot >= 0) {
			player.performAnimation(new Animation(829));
			player.getInventory().getItems()[slot] = new Item(replacePotion, 1);
		...
		player.getInventory().refreshItems();
		player.getAttributes().setDropRateTimer(3000);
		player.getPacketSender().sendMessage("<img=10><col=FF7400><shad=0>You have been granted 10% drop rate boost for 30 minutes.");
	}
	public static void drinkDoublePotion(final Player player, int potion, int replacePotion, int slot, int skill,
			boolean super_pot) {
		if (slot >= 0) {
			player.performAnimation(new Animation(829));
			player.getInventory().getItems()[slot] = new Item(replacePotion, 1);
		...
		player.getInventory().refreshItems();
		player.getAttributes().setDoubleDropTimer(3000);
		player.getPacketSender().sendMessage("<img=10><col=FF7400><shad=0>You have been granted double drop boost for 30 minutes.");
	}
	...
	public static void drinkStatPotion(final Player player, int potion, int replacePotion, int slot, int skill,
		...
		boolean extremePot = potion == 195;
		Skill sk = Skill.forId(skill);
		player.getSkillManager().setCurrentLevel(sk,
				player.getSkillManager().getCurrentLevel(sk) + getBoostedStat(player, skill, super_pot, cbPot, extremePot), true);
	}
	...
	private static void eat(Player player, FoodType food, int slot) {
		...
		if (food != null && player.getFoodTimer().elapsed(1300)) {
			if(food != FoodType.STARTER_BANDAGES) {
				...
				if (heal + player.getSkillManager().getCurrentLevel(Skill.CONSTITUTION) > max) {
					...
					player.getInventory().delete(food.item.getId(), 1);
			...
			Sounds.sendSound(player, Sound.EAT_FOOD);
			} else if(player.getFoodTimer().elapsed(10000)) {
				player.getCombatBuilder().incrementAttackTimer(2).cooldown(false);
				player.setCastSpell(null);
				player.getFoodTimer().reset();
				player.getPacketSender().sendInterfaceRemoval();
				...
				player.getPacketSender().sendMessage("You use the bandages.");
				player.setConstitution(player.getConstitution() + heal);
				Sounds.sendSound(player, Sound.EAT_FOOD);
			}
		}
	...
	public static int getBoostedStat(Player player, int skillId, boolean sup, boolean combatPotion, boolean extremePot) {
		Skill skill = Skill.forId(skillId);
		int increaseBy = 0;
		if (sup) {
			increaseBy = (int) (player.getSkillManager().getMaxLevel(skill) * 0.2D);
		} else if(combatPotion){
			increaseBy = (int) (player.getSkillManager().getMaxLevel(skill) * (0.10D)) + 1;
		} else if(extremePot) {
			increaseBy = (int) (player.getSkillManager().getMaxLevel(skill) * (0.35D)) + 1;
		} else {
			increaseBy = (int) (player.getSkillManager().getMaxLevel(skill) * (0.13D)) + 1;
		}
	...
	public static void handlePotion(final Player player, final int itemId, final int slot) {
		...
		if (player.getPotionTimer().elapsed(900)) {
			 ...
			 */
			case 4063:
				drinkDrPotion(player, itemId, 229, slot, 0, false);
				break;
			case 4061:
				drinkDoublePotion(player, itemId, 229, slot, 0, false);
				break;
			case 4062:
				drinkLifestealPotion(player, itemId, 229, slot, 0, false);
				break;
			case 195:
				...
				drinkStatPotion(player, itemId, 229, slot, 5, false);
				drinkStatPotion(player, itemId, 229, slot, 6, false);
				break;
			...
			if (potion.endsWith("(4)")) {
				player.getPacketSender()
						.sendMessage("You drink some of your " + potion + ". You have 3 doses of potion left.");
			} else if (potion.endsWith("(3)")) {
				player.getPacketSender()
						.sendMessage("You drink some of your " + potion + ". You have 2 doses of potion left.");
			} else if (potion.endsWith("(2)")) {
				player.getPacketSender()
						.sendMessage("You drink some of your " + potion + ". You have 1 dose of potion left.");
			} else if (potion.endsWith("(1)")) {
				player.getPacketSender()
						.sendMessage("You drink some of your " + potion + ". You have finished your potion.");
	...
	public static boolean isPotion(int itemId) {
		String pot = ItemDefinition.forId(itemId).getName();
		return pot.contains("(4)") || pot.contains("(3)") || pot.contains("(2)") || pot.contains("(1)") || pot.contains("potion");
	}
