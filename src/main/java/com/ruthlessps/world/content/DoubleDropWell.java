...
public class DoubleDropWell {
	...
	public static void save() {
		try {
			...
			File file = new File("data/saves/ddr-well.txt");
	...
	public static void donate(Player player, int amount) {
		...
		if(amount >= 100) {
			World.sendMessage("@blu@[SERVER]@or2@ "+player.getUsername()+"@blu@ has contributed "+amount/20+" minutes to Double Drops Well");
		}
	...
	public static void init() {
		...
		TaskManager.submit(new Task(1, false) {
			@Override
			public void execute() {
				if(DoubleDropWell.WELL_TIMER > 0) {
					DoubleDropWell.WELL_TIMER--;
					if(	DoubleDropWell.WELL_TIMER%100 == 0) {
						for(Player p: World.getPlayers()) {
							if(p != null) {
								PlayerPanel.refreshPanel(p);
							}
					...
					if(DoubleDropWell.WELL_TIMER == 0) {
						World.sendMessage("<img=10><col=FF7400><shad=0>The Double Drop Well has run out!");
					}
				}
