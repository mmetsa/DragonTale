...
public class KillsTracker {
	...
	public static int getIndex(Player player, String npcName) {
		for(int i = 0; i < player.getKillsTracker().size(); i++) {
			if(player.getKillsTracker().get(i).npcName.equalsIgnoreCase(npcName)) {
				return i;
			}
		}
		return -1;
	...
	public static void displayExamine(Player player, String npcName, NpcDefinition npcDef) {
		int index = getIndex(player, npcName);
		int amount = 0;
		if(index != -1) {
			amount = player.getKillsTracker().get(index).amount;
		} else {
			player.getPacketSender().sendMessage(npcDef.getExamine()+" (KC: "+0 + ")");
		}
		if(index >= 0) {
			player.getPacketSender().sendMessage(npcDef.getExamine()+" (KC: "+amount+")");
		}
	}
	...
	public static void open(Player player) {
		try {
			...
			player.getPacketSender().sendTabInterface(GameSettings.PLAYER_PANAL, 55000);
