...
public class Graveyard {
	...
	private final static void spawn(Player player, int wave, int level) {
		if(level != 1) {
			player.getInventory().add(14667, 2);
		}
		if (level == 10) {
			leave(player);
			player.getInventory().add(14667, 35);
	...
	public static void start(Player player) {
		if(Objects.nonNull(player.getSummoning().getFamiliar())){
			player.sendMessage("@red@You must first dismiss your familiar");
			return;
		}
		...
		spawn(player, 1, 0);
		Achievements.doProgress(player, AchievementData.PLAY_THE_ZOMBIE_MINIGAME);
		player.getPacketSender().sendMessage("<col=FF0000><shad=0> To leave the graveyard, simply teleport out.");
	}
}
