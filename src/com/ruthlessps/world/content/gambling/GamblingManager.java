package com.ruthlessps.world.content.gambling;
...
import com.ruthlessps.model.input.impl.HandleGambleOfferInput;
import com.ruthlessps.net.packet.PacketBuilder;
import com.ruthlessps.net.packet.interaction.PacketInteraction;
...
import com.ruthlessps.world.content.gambling.gamble.impl.ABCFlowersGamblingGame;
import com.ruthlessps.world.content.gambling.gamble.impl.BlackJackGamblingGame;
import com.ruthlessps.world.content.gambling.gamble.impl.DiceDuelGamblingGame;
import com.ruthlessps.world.content.gambling.gamble.impl.FiftyFiveGamblingGame;
import com.ruthlessps.world.content.gambling.gamble.impl.FlowerPokerGamblingGame;
import com.ruthlessps.world.content.gambling.gamble.impl.FrostyFlowerGamblingGame;
import com.ruthlessps.world.content.gambling.gamble.impl.HotOrColdGamblingGame;
...
public class GamblingManager extends PacketInteraction {
	...
	public static void sendInvitation(final Player player, Player otherPlayer) {
		...
		if(player.getAttributes().getPet() != null || otherPlayer.getAttributes().getPet() != null) {
			player.getPacketSender().sendMessage("Familiars need to be dismissed before gamling!");
			return;
		}
	...
	private static void acceptGameSelection(Player player) {
		...
		if (player.getGambling().getType() == null) {
			player.getPacketSender().sendMessage("You need to select a game first. (FP-ing is currently disabled)");
			return;
		}
		/**
	 ...
	 *            the player
	 */
	public static void cancelGamble(Player player) {
	...
	private static boolean canGamble(final Player player, GambleStage requiredStage) {
		...
		boolean inTrade = TradeManager.SINGLETON.contains(player);
		/**
		 * Decline trade
		 */
		if (inTrade) {
			TradeManager.SINGLETON.getSession(player).ifPresent(session -> session.end(player));
			return false;
		}
		/**
		 * Remove interfaces
	...
	public static boolean handleGambleOffer(Player player, int interfaceId, Item item, int slot) {
		...
		case OFFER_INVENTORY:
			...
			if(player.getGambling().getStage().equals(GambleStage.IN_PROGRESS)) {
				cancelGamble(player);
				removeFlowers(player);
				removeFlowers(otherPlayer);
				Jail.jailPlayer(player);
				player.sendMessage("You were jailed because you were trying to dupe!");//player.getGambleOffer().switchItem(player.getInventory(), item, slot, false, true);
			} else {
		...
		case 3322:
			if (player.getLocation() == Location.GAMBLING_ZONE && !player.getTrading().inTrade()) {
				...
				if(!item.tradeable()) {
					player.getPacketSender().sendMessage("You can not gamble this item because its untradeable!");
					return false;
				}
