...
public class WellOfGoodwill {
	...
	public static void save() {
		try {
			...
			File file = new File("data/saves/edgeville-well.txt");
	...
	public static void donate(Player player, int amount) {
		...
		if(amount >= 100) {
			World.sendMessage("@blu@[SERVER]@or2@ "+player.getUsername()+"@blu@ has contributed "+amount/20+" minutes to Well of Goodwill");
		}
	...
	public static void init() {
		...
		TaskManager.submit(new Task(1, false) {
			@Override
			public void execute() {
				if(WellOfGoodwill.WELL_TIMER > 0) {
					WellOfGoodwill.WELL_TIMER--;
					if(	WellOfGoodwill.WELL_TIMER%100 == 0) {
						for(Player p: World.getPlayers()) {
							if(p != null) {
								PlayerPanel.refreshPanel(p);
							}
					...
					if(WellOfGoodwill.WELL_TIMER == 0) {
						World.sendMessage("<img=10><col=FF7400><shad=0>The Well of Goodwill has run out!");
					}
				}
