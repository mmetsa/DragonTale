...
public class PrayerHandler {
	 ...
	 * @param prayerId
	 *            The id of the prayer being turned on, also known as the ordinal in
	 *            the respective enum.
	 */
	...
	private static final double getDrain(Player player) {
		...
		if (toRemove > 0) {
			toRemove /= (1 + (0.05 * (player.getBonusManager().getOtherBonus()[2]+player.getAttributes().getPrayerBonus())));
		}
		return toRemove;
	}
