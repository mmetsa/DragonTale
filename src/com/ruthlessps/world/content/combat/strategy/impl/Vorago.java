...
public class Vorago implements CombatStrategy {
	...
	public boolean customContainerAttack(Character entity, Character victim) {
		...
		if (Locations.goodDistance(vortex.getPosition().copy(), victim.getPosition().copy(), 1)
			...
			vortex.getCombatBuilder().setContainer(new CombatContainer(vortex, victim, 1, 1, CombatType.MELEE, true));
		} else if(Locations.goodDistance(vortex.getPosition().copy(), victim.getPosition().copy(), 1)
				&& rand <= 7) {
			vortex.performAnimation(new Animation(vortex.getDefinition().getAttackAnimation()));
			vortex.getCombatBuilder().setContainer(new CombatContainer(vortex, victim, 1, 1, CombatType.RANGED, true));
		} else {
			vortex.performAnimation(new Animation(vortex.getDefinition().getAttackAnimation()));
			vortex.getCombatBuilder().setContainer(new CombatContainer(vortex, victim, 1, 1, CombatType.MAGIC, true));
			victi.getSkillManager().setCurrentLevel(Skill.PRAYER, (int) (victi.getSkillManager().getCurrentLevel(Skill.PRAYER)-100), true);
			if(victi.getSkillManager().getCurrentLevel(Skill.PRAYER) <= 100) {
				victi.getPacketSender().sendMessage("Vorago has completely drained your prayer!");
				victi.getSkillManager().setCurrentLevel(Skill.PRAYER, 0);
