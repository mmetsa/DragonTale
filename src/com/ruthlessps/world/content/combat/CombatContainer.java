...
public class CombatContainer {
	...
	public final void dealDamage() {
		if(victim.isNpc() && ((NPC)victim).getId() != 453) {
			if(attacker.isPlayer()) {
				Player player = (Player)attacker;
				if (hits.length == 1) {
					victim.dealDamage(hits[0].getHit());
					player.setNpcDamage(player.getNpcDamage()+hits[0].getHit().getDamage());
				} else if (hits.length == 2) {
					victim.dealDoubleDamage(hits[0].getHit(), hits[1].getHit());
					player.setNpcDamage(player.getNpcDamage()+hits[0].getHit().getDamage()+hits[1].getHit().getDamage());
				} else if (hits.length == 3) {
					victim.dealTripleDamage(hits[0].getHit(), hits[1].getHit(), hits[2].getHit());
					player.setNpcDamage(player.getNpcDamage()+hits[0].getHit().getDamage()+hits[1].getHit().getDamage()+hits[2].getHit().getDamage());
				} else if (hits.length == 4) {
					victim.dealQuadrupleDamage(hits[0].getHit(), hits[1].getHit(), hits[2].getHit(), hits[3].getHit());
					player.setNpcDamage(player.getNpcDamage()+hits[0].getHit().getDamage()+hits[1].getHit().getDamage()+hits[2].getHit().getDamage()+hits[3].getHit().getDamage());
				}
			} else {
				if (hits.length == 1) {
					victim.dealDamage(hits[0].getHit());
		...
		} else if(victim.isNpc() && ((NPC)victim).getId() == 453) {
			Player player = (Player)attacker;
			if (hits.length == 1) {
				victim.dealFakeDamage(hits[0].getHit());
				player.setDummyDamage(player.getDummyDamage()+hits[0].getHit().getDamage());
			} else if (hits.length == 2) {
				victim.dealFakeDoubleDamage(hits[0].getHit(), hits[1].getHit());
				player.setDummyDamage(player.getDummyDamage()+hits[0].getHit().getDamage()+hits[1].getHit().getDamage());
			} else if (hits.length == 3) {
				victim.dealFakeTripleDamage(hits[0].getHit(), hits[1].getHit(), hits[2].getHit());
				player.setDummyDamage(player.getDummyDamage()+hits[0].getHit().getDamage()+hits[1].getHit().getDamage()+hits[2].getHit().getDamage());
			} else if (hits.length == 4) {
				victim.dealFakeQuadrupleDamage(hits[0].getHit(), hits[1].getHit(), hits[2].getHit(), hits[3].getHit());
				player.setDummyDamage(player.getDummyDamage()+hits[0].getHit().getDamage()+hits[1].getHit().getDamage()+hits[2].getHit().getDamage()+hits[3].getHit().getDamage());
			}
		} else {
			if (hits.length == 1) {
				victim.dealDamage(hits[0].getHit());
	...
	private final CombatHit[] prepareHits(int hitAmount) {
		...
		if (attacker.isPlayer() && ((Player) attacker).isSpecialActivated()) {
			if (((Player) attacker).getCombatSpecial() == CombatSpecial.DRAGON_CLAWS && hitAmount == 4) {
				...
				int fourth = first <= 0 && second <= 0 && third <= 0 ? array[3].getHit().getDamage() + Misc.getRandom(7)
