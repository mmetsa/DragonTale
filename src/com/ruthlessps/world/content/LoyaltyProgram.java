...
public class LoyaltyProgram {
		...
		LOYALIST(-1, 43043, -22496) {
			...
			boolean canBuy(Player p, boolean sendMessage) {
				if (!p.getUnlockedLoyaltyTitles()[9]) {
					if (p.getPointsManager().getPoints("loyalty") < 100000) {
						if (sendMessage)
							p.getPacketSender().sendMessage(
		...
		VETERAN(-1, 43047, -22492) {
			...
			boolean canBuy(Player p, boolean sendMessage) {
				if (!p.getUnlockedLoyaltyTitles()[10]) {
					if (p.getAchievementAttributes().getTotalLoyaltyPointsEarned() < 2500000) {
						if (sendMessage)
							p.getPacketSender().sendMessage(
									"To unlock this title, you must have earned 2,500,000 Loyalty Points in total.");
						return false;
					}
				}
				unlock(p, this);
			...
			boolean canBuy(Player p, boolean sendMessage) {
				if (!p.getUnlockedLoyaltyTitles()[11]) {
					if (!p.getInventory().contains(15084)) {
						if (sendMessage)
							p.getPacketSender()
									.sendMessage("To unlock this title, you must have a Dice in your inventory.");
						return false;
					}
				}
				unlock(p, this);
	...
	public static boolean handleButton(Player player, int button) {
		...
		if (title != null) {
			if (title.canBuy(player, true)) {
				...
				if (player.getPointsManager().getPoints("loyalty") >= title.cost) {
					player.setLoyaltyTitle(title);
					player.getPointsManager().decreasePoints("loyalty", title.cost);
	...
	public static void incrementPoints(Player player) {
		...
		player.getPointsManager().setWithIncrease("loyalty", (int) pts); // try this
		player.getAchievementAttributes().incrementTotalLoyaltyPointsEarned(pts);

		int totalPoints = player.getPointsManager().getPoints("loyalty");
		...
		player.getPacketSender().sendString(39176, "@red@Loyalty Points: @whi@" + totalPoints);
		if (totalPoints >= 2500000) {
	...
	public static void open(Player player) {
		...
		player.getPacketSender()
				.sendString(43120, "Your Loyalty Points: " + player.getPointsManager().getPoints("loyalty"))
				.sendInterface(43000);
	}
	...
	public static void unlock(Player player, LoyaltyTitles title) {
		if (player.getUnlockedLoyaltyTitles()[title.ordinal()])
			return;
		Achievements.doProgress(player, AchievementData.LOYALTY_TITLES);
