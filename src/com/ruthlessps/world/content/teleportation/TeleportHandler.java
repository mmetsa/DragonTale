...
public class TeleportHandler {
	...
	public static boolean checkReqs(Player player, Position targetLocation) {
		if (player.isJailed == 1) {
			player.getPacketSender().sendMessage("You are jailed and can not teleport!");
			return false;
		}
		...
		if(KillGame.isInWaitArea(player)) {
			KillGame.leaveWaitingArea(player);
			return true;
		}
			...
			public void execute() {
				...
				case LEVER:
					if (tick == 0)
						...
						player.getSummoning().moveFollower(true);
						stop();
					}
					break;
				default:
