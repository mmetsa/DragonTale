package com.ruthlessps.world.content.skill.impl.fletching;
...
import com.ruthlessps.model.input.impl.EnterAmountOfBowsToString;
import com.ruthlessps.model.input.impl.EnterAmountToFletch;
...
public class Fletching {
	...
	private static int BLOOD_STRING = 20082;
	...
	public static boolean fletchingButton(final Player player, int button) {
		...
		case 8874:
			...
			case 1513: // Magic logs
				...
				return true;
			case 20075: // Blood logs
				fletchBow(player, 20073, 1);
				return true;
			}
			return false;
		case 8873:
		...
		if (arr != null) {
			if (player.getSkillManager().getCurrentLevel(Skill.FLETCHING) >= arr.getLevelReq()) {
				if (player.getInventory().getAmount(arr.getItem1()) >= 15
					...
					Achievements.doProgress(player, AchievementData.FLETCH_SOME_ARROWS);
					Achievements.doProgress(player, AchievementData.FLETCH_50_ARROWS);
					Achievements.doProgress(player, AchievementData.FLETCH_500_ARROWS);
				} else {
