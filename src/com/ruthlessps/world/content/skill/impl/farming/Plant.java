		...
		if (player.getInventory().contains(FarmingPatches.values()[patch].harvestItem)) {
			...
			TaskManager.submit(new Task(1, player, true) {
				...
				public void execute() {
					...
					if(player.getGameMode() == GameMode.HARDCORE && player.getEquipment().contains(9810)) {
						add = ItemDefinition.forId(id).isNoted() ? new Item(id - 1, 2) : new Item(id, 2);
					} else {
					...
					Achievements.doProgress(player, AchievementData.HARVEST_A_CROP);
					...
					Achievements.doProgress(player, AchievementData.HARVEST_A_CROP);
					Achievements.doProgress(player, AchievementData.HARVEST_50_CROPS);
					Achievements.doProgress(player, AchievementData.HARVEST_500_CROPS);
					harvested++;
	...
	public void process(Player player) {
		...
		if (elapsed >= grow) {
			for (int i = 0; i < elapsed / grow; i++) {
				/*
				 * if (isDiseased()) { /*} else if (!isWatered()) { player.getPacketSender().
				 * sendMessage("You need to water your plant.");
				 */
				if (isWatered()) {
					stage++;
					player.getFarming().doConfig();
					if (stage >= Plants.values()[plant].stages) {
						player.getPacketSender()
								.sendMessage("<shad=996699>A seed which you planted has finished growing!");
						return;
					}
				}
