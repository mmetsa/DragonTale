package com.ruthlessps.world.content.skill.impl.dungeoneering;
...
import com.ruthlessps.world.content.dialogue.impl.DungPartyInvitation;
...
public class DungeoneeringParty {
	...
	public static void create(Player p) {
		...
		p.getPacketSender().sendMessage(
				"<col=660000>You've created a Dungeoneering party. Perhaps you should invite a few players?");
	...
	public void remove(Player p, boolean resetTab, boolean fromParty) {
		if (fromParty) {
			...
			if (resetTab) {
				p.getPacketSender().sendTabInterface(GameSettings.QUESTS_TAB, p.isKillsTrackerOpen() ? 55000 : 6387);
		...
		if (hasEnteredDungeon) {
			...
			int exp = (int) (damage > 0
					? (p.getMinigameAttributes().getDungeoneeringAttributes().getDamageDealt() * 3.5)
							- (deaths * 2800 + Misc.getRandom(400))
					: 0);
			int tokens = (int) (damage > 0
					? (p.getMinigameAttributes().getDungeoneeringAttributes().getDamageDealt() / 3.8)
							- (deaths * 80 + Misc.getRandom(25))
					: 0);
			if (killedBoss) {
				exp += 9000 + Misc.getRandom(2000);
				tokens += Misc.getRandom(300);
			}
			if (exp > 0 && tokens > 0) {
				exp += (2000 + Misc.getRandom(1000)) * complexity;
				if (player_members.size() == 1) {
					exp = (int) (exp * 0.7);
					tokens = (int) (tokens * 0.7);
				}
				p.getSkillManager().addExperience(Skill.ATTACK, exp);
				p.getPointsManager().setWithIncrease("dung", tokens);
				p.getPacketSender().sendMessage("<col=660000>You've received some Dungeoneering experience and "
	...
	public void sendMessage(String message) {
		for (Player member : getPlayers()) {
			if (member != null) {
				member.getPacketSender().sendMessage("<col=660000>" + message);
			}
		}
	}
