...
public class DialogueManager {
	...
	/**
	 * Custom npc's with no chat model head go here
	 * 
	 * @param npcId
	 * @return
	 */
	private static boolean isCustomNPC(int npcId) {
		switch (npcId) {
		case 6139:
		case 4657:
			return true;
	...
	private static void showDialogue(Player player, Dialogue dialogue) {
		...
		case NPC_STATEMENT:
			...
			if (isCustomNPC(dialogue.npcId())) {
				player.getPacketSender().sendNpcHeadOnInterface(4145, headChildId);
			} else {
