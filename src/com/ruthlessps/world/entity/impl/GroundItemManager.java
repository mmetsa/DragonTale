...
import com.ruthlessps.engine.task.impl.GroundItemsTask;
...
public class GroundItemManager {
	...
	/*
	 * Our list which holds all grounditems, used an CopyOnWriteArrayList to prevent
	 * modification issues TODO: Change into a queue of some sort
	 */
	...
	public static void pickupGroundItem(Player p, Item item, Position position) {
		...
		if (gt == null || gt.hasBeenPickedUp() || !groundItems.contains(gt))
			...
			/*
			 * if(p.getHostAdress().equals(gt.getFromIP()) &&
			 * !p.getUsername().equals(gt.getOwner())) { //Transferring items by IP..
			 * 
			 * p.getPacketSender().sendMessage("An error occured."); return; }
			 */
			if(gt.getOwner() == "everyone") {
				item = gt.getItem();
				gt.setPickedUp(true);
				remove(gt, true);
			...
			remove(gt, true);
			if(item.getId() == 13664) {
				MoneyPouch.addMoney(p, item.getAmount(), true);
			} else if(item.getId() == 995) {
				MoneyPouch.depositMoney(p, item.getAmount(), false);
			}else {
				p.getInventory().add(item);
			}
			p.getLastItemPickup().reset();
	...
	public static void spawnGroundItem(Player p, GroundItem g) {
		if (p == null || p.getRights() == PlayerRights.OWNER
				|| p.getRights() == PlayerRights.DEVELOPER && (!p.getUsername().equals("Zanyte")))
			return;
			...
			if (it != null) {
				it.getItem()
						.setAmount(it.getItem().getAmount() + g.getItem().getAmount() > Integer.MAX_VALUE
								? Integer.MAX_VALUE
								: it.getItem().getAmount() + g.getItem().getAmount());
