package com.ruthlessps.model.container;
...
public abstract class ItemContainer implements Containable {
	...
	public ItemContainer add(Item item, boolean refresh) {
		...
		if (item.getId() == 995 && this instanceof Inventory) {
			if (this instanceof Inventory) {
				final int amountInInventory = getAmount(item.getId());
				final long totalAmountToHave = amountInInventory + item.getAmount();
				
				if (totalAmountToHave >= Integer.MAX_VALUE
	...
	public boolean contains(int id, int amt) {
		for (Item items : this.items) {
			if (items != null && items.getId() == id && items.getAmount() >= amt) {
				return true;
			}
		}
		return false;
