...
//import com.ruthlessps.model.input.impl.ItemSearch;
...
public class Bank extends ItemContainer {
	...
	public static class BankSearchAttributes {
		...
		public static void beginSearch(Player player, String searchSyntax) {
			...
			// searchSyntax = (String)
			// ItemSearch.getFixedSyntax(searchSyntax)[0];
		...
		public static void stopSearch(Player player, boolean openBank) {
			...
			player.getPacketSender().sendString(27002, Integer.toString(0)).sendString(27000, "1").sendConfig(117, 0)
					.sendString(5383, "        The Bank of DragonTale");
			if (openBank)
	...
	public static void sendTabs(Player player) {
		 ...
		 * if(isEmpty(player.getBank(i)) || moveRest) { int j = i+2 > 8 ? 8 : i+2;
		 * player.setBank(i, player.getBank(j)); player.setBank(j, new Bank(player));
		 * moveRest = true; } }
		 */
		int l = 1;
		for (int i = 22035; i < 22041; i++) {
			player.getPacketSender().sendItemOnInterface(i, player.getBank(l).get(0).getId(), 1);
			l++;
		}
