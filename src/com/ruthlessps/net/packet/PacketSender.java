...
import com.ruthlessps.model.PlayerInteractingOption;
...
import com.ruthlessps.net.packet.Packet.PacketType;
...
import com.ruthlessps.world.content.kaleem.trade.Containable;
import com.ruthlessps.world.content.kaleem.trade.TradeManager;
import com.ruthlessps.world.content.skill.impl.construction.ConstructionData.Furniture;
import com.ruthlessps.world.content.skill.impl.construction.Palette;
import com.ruthlessps.world.content.skill.impl.construction.Palette.PaletteTile;
...
public class PacketSender {
	...
	public PacketSender sendInterfaceRemoval() {
		...
		if(player.isHasOpenPOSEditor()){
			player.setHasOpenPOSEditor(false);
		}
		sendAnimationReset();
	...
	public PacketSender sendItemContainer(Containable container, int interfaceId) {
		PacketBuilder out = new PacketBuilder(53, PacketType.SHORT);
		out.putShort(interfaceId);
	...
	public PacketSender sendItemOnInterface(int item, int slot, int column, int amount) {
		PacketBuilder out = new PacketBuilder(34, PacketType.SHORT);
		out.putShort(column);
		out.put(slot);
		out.putShort(item + 1);
	...
	//Might be wrong
	public PacketSender sendRewards(ArrayList<MysteryBoxRewards> rewards) {
		PacketBuilder out = new PacketBuilder(128, PacketType.SHORT);
		for(MysteryBoxRewards reward : rewards) {
			//player.getPacketSender().sendMessage("itemName:"+ItemDefinition.forId(reward.reward.getId() + 1).getName());
			out.putShort(reward.reward.getId() + 1);
			out.putInt(reward.reward.getAmount());
			out.put(reward.rarity.ordinal());
		}
		player.getSession().queueMessage(out);
		return this;
	}
	
	public PacketSender sendWinnerIndex(int winnerIndex) {
	...
	public PacketSender sendRights() {
		...
		out.put(player.getDonor().ordinal());
		player.getSession().queueMessage(out);
		return this;
	}
	...
	public PacketSender sendTabs() {
		...
		sendTabInterface(GameSettings.QUESTS_TAB, 6939);//6939
		...
		sendTabInterface(GameSettings.PLAYER_PANAL, 6387);
		sendTabInterface(15, 46343);
		return this;
	}
