...
import com.ruthlessps.net.packet.codec.PacketDecoder;
import com.ruthlessps.net.packet.codec.PacketEncoder;
import com.ruthlessps.net.security.IsaacRandom;
	...
	protected Object decode(ChannelHandlerContext ctx, Channel channel, ChannelBuffer buffer) throws Exception {
		...
		case LOGGING_IN:
			...
			if (seedReceived != seed) {
				// System.out.println("Unhandled seed read: [seed, seedReceived]
				// : [" + seed + ", " + seedReceived + "]");
				channel.close();
				return null;
			}
			...
			return login(channel,
					new LoginDetailsMessage(username, password,
							((InetSocketAddress) channel.getRemoteAddress()).getAddress().getHostAddress(), serial, mac,
							clientVersion, uid));
		}
		return null;
	}
	...
	public Player login(Channel channel, LoginDetailsMessage msg) {
		...
		if (response == LoginResponses.LOGIN_SUCCESSFUL) {
			channel.write(new PacketBuilder().put((byte) 2).put((byte) player.getRights().ordinal())
					.put((byte) player.getDonor().ordinal()).

					put(player.isModeler() ? 1 : 0).put(player.isGambler() ? 1 : 0).put(player.isGfxDesigner() ? 1 : 0)
					.put(player.isYoutuber() ? 1 : 0).
