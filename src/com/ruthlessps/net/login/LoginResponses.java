...
public final class LoginResponses {
	...
	/**
	 * This login opcode is used for a 2000ms delay, after which a reconnection is
	 * attempted.
	 */
	 ...
	 * exceeded {@link org.desolace.net.NetworkConstants.MAXIMUM_CONNECTIONS}.
	 */
	...
	/**
	 * This login opcode is used when a non-member player is attempting to login to
	 * a members world.
	 */
	...
	/**
	 * This login opcode is received upon a reconnection so that chat messages will
	 * not dissappear.
	 */
	...
	/**
	 * This login opcode is used when a player has just left another world and has
	 * to wait a delay before entering a new world.
	 */
	...
	public static final int getResponse(Player player, LoginDetailsMessage msg) {
		...
		if (PlayerPunishments.isMacBanned(msg.getMac())) {
		...
		if (playerLoadingResponse == LoginResponses.NEW_ACCOUNT
				&& (msg.getUsername().toLowerCase().contains("ruthless"))) {
